...
class ScanContext(BaseModel):
    ...
    def get_secondary_headers(self) -> Dict[str, str]:
        """Return headers with secondary auth token injected."""
        final_headers = self.headers.copy()
        if self.auth.secondary_token:
            final_headers[self.auth.header_name] = f"{self.auth.token_type} {self.auth.secondary_token}"
        return final_headers
