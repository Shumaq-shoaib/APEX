...
class PayloadLibrary:
    ...
    SSTI_PAYLOADS = ["{{7*7}}", "${7*7}", "<%= 7*7 %>", "#{7*7}", "{{7*'7'}}", "{{config}}", "${class.forName('java.lang.Runtime')}"]
    CORS_EVIL_ORIGINS = ["http://evil.com", "https://evil.com", "null"]
    CSRF_HEADER_NAMES = ["X-CSRF-Token", "X-XSRF-Token", "X-CSRFToken", "csrf-token", "csrf_token", "_csrf"]
    RATE_LIMIT_SENSITIVE_PARAMS = ["password", "pin", "otp", "cvv", "pass", "secret"]
